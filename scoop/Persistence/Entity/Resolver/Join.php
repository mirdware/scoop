...
class Join
{
    ...
    public function __construct($mapper, $map, Field $fieldResolver)
    {
        $this->mapper = $mapper;
        $this->map = $map;
        ...
        foreach ($aggregatesPath as $propertyName) {
            ...
            if (isset($entityMap['properties'][$propertyName])) {
                ...
                if ($relation[2] === \Scoop\Persistence\Entity\Relation::MANY_TO_MANY) {
                    ...
                    $this->fieldResolver->addJoin($relation['table'] . ' ' . $leftAlias . $rightAlias, $comparation, $joinType);
            ...
            $aggregateList = &$aggregateList[$propertyName]['aggregates'];
            $entityMap = $this->map['entities'][$left];
            $this->fieldResolver->addFields($aggregate, $rightAlias);
        }
    }
}
